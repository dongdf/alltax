...
<template>
  <div>
    <div id="course-detail">
      ...
      <video id="player-container-id" v-if="courseInfo.key == 2 "  preload="auto"   x5-playsinline playsinline webkit-playsinline></video>

      <div      id="neplayer" class="neplayercontent"></div>
  ...
  export default {
    ...
    methods: {
     ...
     // <video id="player-container-id" v-if="courseInfo.source == 2"  preload="auto"   x5-playsinline playsinline webkit-playsinline></video>
      ...
      qqPlay(){
        var that = this;
        this.$http.post('curriculum/curriculumService/getPlayCommenAuth',{subData:this.$route.query.id}).then(res=> {
          if (res.data.code == 1) {
...
this.pinfo =res.data.data;
      ...
      createVideo(){
        ...
        container.innerHTML="";
        container.innerHTML = '<video id="my-video" x5-playsinline  playsinline="" webkit-playsinline="" autoplay class="" controls  width="100%"   data-setup=\'{}\'></video>';
      ...
      getcoursedetail(){//请求课程数据
        this.$http.post('curriculum/curriculumService/getPlayCommenAuth', {subData:this.$route.query.id}).then(res => {
          ...
          if(res.data.code==1){
            ...
            if(this.courseInfo.key == 1){
              // this.aliPlayXin();
              this.createVideo();
              this.playVideo.src=this.courseInfo.videoId;
              this.playVideo.play();
              // this.qqPlay();
              // this.playClick();
            }else{
